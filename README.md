...
- **前端**：原生 HTML / CSS / JS，无构建工具
  - ECharts（打卡热力图、学习趋势、正确率图表）
  - Phosphor Icons（图标体系）

### 技术亮点

- **词汇画像驱动的 AI 写作**：以"已掌握单词"为输入构建用户词汇画像，让 AI 生成的范文与专属模板严格落在用户当前词汇水平内，并按场景为导入 / 速记 / 默写 / 小助手 / 写作工坊分别指定模型
- **AI 调用全链路可观测**：每次 LLM 调用（模型、耗时、成败、错误）落库留存，写作工坊与 AI 小助手共用一套调用与日志基建
- **结构化 AI 输出**：作文批改 / 翻译批改 / 命题分析均要求 LLM 返回严格 JSON，前端按结构渲染五维评分与逐句改错
- **纯前端图表与交互**：ECharts 本地打包离线可用，零前端构建依赖，整体为 Django 模板 + 原生 JS 单页式交互
...
或者参考001文件夹下的使用说明！可以直接运行word.exe文件。
## AI 模型配置
