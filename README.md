...
- **每日打卡（自动）**
  - 每天学满 30 个单词自动打卡，无需手动操作；连续打卡天数展示在仪表盘
- **刷题打卡日历热力图**
  - GitHub 贡献网格风格，颜色梯度：空白 → 浅绿 → 中绿 → 深绿 → 墨绿
  - 支持 1 个月 / 3 个月 / 6 个月 / 1 年 范围切换
  - 按自然月分块显示，月份标题居中，完整周对齐无断层
  - 顶部展示总做题天数、连续打卡天数
- **模拟考试**
  - 可配置考试范围、题数、考试方式
  ...
  - 交卷后自动更新单词掌握状态
- **学习统计**
  - 学习趋势堆叠光滑面积曲线图、正确率、每日打卡、新学 / 复习量统计（ECharts 数据可视化）
- **学习计划**
...
> 也可以使用你的 AI 编程工具，输入"启动项目"，即可启动项目。

## AI 模型配置
进入 `/settings/` 页面进行操作设置，支持 OpenAI 兼容接口（DeepSeek、Gemini、Groq 等）。
