...
- **背诵与复习**
  ...
  - **笔记**：背诵会话中可为每个单词记录笔记（记忆方法、例句、易错点、页码），自动保存到数据库
  - **AI 速记**：背诵会话中一键生成速记（拆分秒背 / 谐音 / 联想口诀），结果缓存到数据库，下次直接查看不再重复调用 AI，也可手动编辑或重新生成
...
AI 相关功能（单词导入识别、AI 复审、AI 速记）统一使用「设置 → AI 模型」中配置的模型，配置保存在数据库中：

1. 进入 `/settings/` →「AI 模型」→ 点击「添加模型」
2. 选择服务商 → 填写 API Key（部分本地服务可留空）→ 选择或填写模型 ID
3. 点击「测试连接」验证可用性，保存后自动启用
4. 之后在 `/ai-import/`（图片 / 文本 / 文件识别）与背诵会话的速记面板中即可直接使用该模型，无需重复配置
...
│   └── urls.py
├── words/                  # 主应用
│   ├── models.py           # Unit / Word / StudyProgress / StudyPlan / StudySession / QuickMemory / AIModel 等
